...
try {
  ...
  const candidate = await page.evaluate(() => {
    ...
    return { x: world.harbour.x + .5, z: world.harbour.z + .5 };
  });
  ...
  await page.waitForFunction(() => window.oikos.state.seed === 2 && window.oikos.state.home === 7);
  assert.equal(await page.evaluate(() => window.oikos.state.founded), false);
