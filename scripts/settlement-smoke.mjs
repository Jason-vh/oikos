...
try {
  ...
  await page.waitForFunction(() => window.oikos.state.seed === 2 && window.oikos.state.cities[0].home === 0);
  await page.getByRole('button', { name: /pause/i }).first().click();
  const landing = (await state(page)).cities[0].harbour;
  ...
  assert.equal(chosen.cities[0].founded, true, 'Founding harbour was not placed');
  const map = await page.evaluate(() => window.oikos.map);
  assert.equal(map.home, 0);
  assert.deepEqual(map.entry, map.islands[0].entry);
  assert(chosen.cities[0].harbour.connected, 'The chosen landing is disconnected');
  ...
  await page.waitForFunction(() => document.body.dataset.ready === 'true');
  await page.getByRole('button', { name: /pause/i }).first().click();
  assert.equal((await state(page)).cities[0].home, 0, 'Reload forgot the chosen island');
  assert.deepEqual((await page.evaluate(() => window.oikos.map)).entry, map.entry);
  assert((await state(page)).cities[0].delivered >= checkpoint.cities[0].delivered, 'Reload lost the economy');
  ...
  await page.waitForFunction(() => window.oikos.state.cities[0].home === 7);
