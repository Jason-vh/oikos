...
import type { Rotation, Tile } from '../sim/types';
import type { AgentGame } from './game';
import { cityReport, cityWindow, describeFounding, describePlacement, describeRoadPath, inspectBuilding, inspectTile, islandBounds, surveyIsland } from './view';
...
function outcome(reason: string, money: number, ok: boolean): string {
  return `${ok ? 'Done.' : 'Refused.'} ${reason} Treasury ${Math.round(money)} dr.`;
}
...
export const TOOLS: AgentTool[] = [
  ...
  tool({
    ...
    schema: { id: tile },
    async run(game, args) {
      return inspectBuilding(game.view().city, args.id);
    },
  }),
  tool({
    name: 'check_build',
  ...
  tool({
    ...
    async run(game, args) {
      ...
      return outcome(result.reason, game.view().city.money, result.ok);
    },
  }),
  tool({
    name: 'lay_road',
