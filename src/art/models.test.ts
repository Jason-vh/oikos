...
function instances(): { kind: BuildingKind; tier: 1 | 2 | 3; vendorEnabled: boolean; variant: number; model: T.Group }[] {
  const result: { kind: BuildingKind; tier: 1 | 2 | 3; vendorEnabled: boolean; variant: number; model: T.Group }[] = [];
      ...
      for (const vendorEnabled of vendorOptions) {
        for (const stage of stages) {
          for (let variant = 0; variant < modelVariants(kind, tier); variant++) {
            result.push({ kind, tier, vendorEnabled, variant, model: getBuildingModel(kind, { tier, vendorEnabled, stores, stage, variant }) });
          }
        }
      }
    }
...
describe('getBuildingModel footprints', () => {
  for (const { kind, tier, vendorEnabled, variant, model } of instances()) {
    test(`${kind} tier ${tier}${vendorEnabled ? ' (vendor)' : ''} variant ${variant} fits its catalog footprint`, () => {
...
describe('getBuildingModel ground contact', () => {
  for (const { kind, tier, vendorEnabled, variant, model } of instances()) {
    test(`${kind} tier ${tier}${vendorEnabled ? ' (vendor)' : ''} variant ${variant} sits on y = 0`, () => {
      const bounds = new T.Box3().setFromObject(model);
...
describe('getBuildingModel vertex integrity', () => {
  for (const { kind, tier, vendorEnabled, variant, model } of instances()) {
    test(`${kind} tier ${tier}${vendorEnabled ? ' (vendor)' : ''} variant ${variant} has only finite vertices`, () => {
...
describe('getBuildingModel drawcall and triangle budgets', () => {
  for (const { kind, tier, vendorEnabled, variant, model } of instances()) {
    test(`${kind} tier ${tier}${vendorEnabled ? ' (vendor)' : ''} variant ${variant} stays within budget`, () => {
