...
import { animateFigure, animateHauling, animateIdle, animateWork, axe, bake, box, bundle, bundleKey, chopStrikes, CHOP_SET, colors, disposeModel, figure, getBuildingAssembly, getBuildingModel, net, post, skiff, spear, variantFor, workPeriod, type Idle, type ModelStage, type WorkKind } from '../art';
...
export class CityScene {
  ...
  sync(world: World): void {
    ...
    for (const building of buildings) {
      ...
      const key = `${building.kind}:${building.tier}:${stallKey(building)}:${stage}:${storesKey(building)}:${variant}:${building.rotation}:${building.x}:${building.z}`;
      ...
      const state = { tier: building.tier, stalls: building.stalls, stage, stores: building.stores, variant };
