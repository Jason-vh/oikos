...
test('one remaining socket advances both cities; tick checkpoints, last close, restart and empty time never catch up', async () => {
  ...
  expect((await second.peer.next('snapshot')).world.time).toBeCloseTo(5);
  f.clock.step(250);
  ...
  expect(withoutWildlife(durable(f.path).world)).toEqual(withoutWildlife(settled.world));
  f.clock.step(1000);
  ...
  expect(withoutWildlife(reconnected.snapshot.world)).toEqual(withoutWildlife(settled.world));
