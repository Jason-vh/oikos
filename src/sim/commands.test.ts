...
test('JSON commands found and grow exactly the same city as the direct simulation API', () => {
  ...
  execute({ type: 'foundHarbour', x: world.harbour.x, z: world.harbour.z });
...
test('well-formed commands still enforce founding and territory rules', () => {
  ...
  expect(applyCommand(world, { type: 'foundHarbour', x: world.harbour.x, z: world.harbour.z }).ok).toBe(true);
  const before = serializeWorld(world);
