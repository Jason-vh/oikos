...
export function suitableFarmGround(world: World): Tile[] {
  ...
  for (const road of primaryCity(world).roads) occupied.add(road);
  const tiles: Tile[] = [];
...
export function demolitionPreview(world: World, x: number, z: number): DemolitionPreview | null {
  ...
  const building = primaryCity(world).buildings.find((candidate) => buildingFootprintTiles(map, candidate).includes(tile));
  if (building) {
  ...
  if (primaryCity(world).roads.includes(tile)) return { tile, buildingId: null, kind: 'road', refund: 0, footprint: [tile] };
  return null;
}
