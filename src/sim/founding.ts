...
export function foundingPlacement(world: World, x: number, z: number): Placement {
  if (primaryCity(world).founded) return { ok: false, reason: 'This city already has its founding harbour.', cost: 0, tiles: [] };
  ...
  const candidate = { ...primaryCity(world).harbour, x, z };
