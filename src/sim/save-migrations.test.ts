...
describe('v4-v8 saves migrate their one city into world.cities', () => {
  ...
  test('a matching deserialized world preserves every City field and the shared clock, allocator, entities and wildlife exactly', () => {
    ...
    for (const version of [4, 5, 6, 7, 8]) {
      ...
      expect(city.roads).toEqual(original.roads);
      expect(city.buildings).toEqual(original.buildings);
      expect(city.walkers).toEqual(original.walkers);
...
describe('malformed version 8 cities are refused, not silently accepted', () => {
  const badCities = [undefined, null, [], {}, 'city', [{ id: 1 }, { id: 2 }], [[]], [null], [42]];

  test('migrateSave rejects every malformed shape without throwing', () => {
    const raw = currentRaw();
...
describe('never silently accepting corrupted fields', () => {
  ...
  test('a present but invalid walker working timer is rejected, not defaulted', () => {
    ...
    raw.cities[0].walkers[0].working = 'a while';
    expect(deserializeWorld(JSON.stringify(raw))).toBeNull();
  });

  test('a building using the pre-stores stock field is rejected, not reinterpreted', () => {
    const raw = currentRaw();
    if (raw.cities[0].buildings.length === 0) return;
    delete raw.cities[0].buildings[0].stores;
    raw.cities[0].buildings[0].stock = 40;
    expect(deserializeWorld(JSON.stringify(raw))).toBeNull();
  });
